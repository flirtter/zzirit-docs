...
> **시드 스크립트는 메인 유저 프로필을 절대 수정하지 않는다.**
> 유저가 온보딩에서 직접 입력한 데이터(이름, 직업, 이미지, MBTI 등)가 SSOT.
> `users` 컬렉션이 온보딩 원본, `profile` 컬렉션은 이를 반영.
> 시드 스크립트는 seed_user_XX 더미 유저만 생성하고, 메인 유저는 볼트 잔액만 설정.
