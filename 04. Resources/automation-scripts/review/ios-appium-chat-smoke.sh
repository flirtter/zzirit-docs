...
REPORT_ROOT="${ZZIRIT_APPIUM_REPORT_DIR:-$PROJECT_ROOT/artifacts/appium-chat}"
...
if [ -z "$SIM_UDID" ]; then
  echo "[ios-appium-chat] Could not resolve simulator: $SIM_NAME"
  exit 1
fi
...
if ! curl -fsS "${APPIUM_SERVER_URL}/status" >/dev/null 2>&1; then
  ...
  wait_for_appium || {
    echo "[ios-appium-chat] appium server did not become ready"
    exit 1
  }
fi
...
node "$PROJECT_ROOT/scripts/review/ios-appium-chat-smoke.mjs"
